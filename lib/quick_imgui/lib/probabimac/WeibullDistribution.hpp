...
public:
    WeibullDistribution(double shape_k, double scale_lambda) : _k(shape_k), _lambda(scale_lambda), _uniform(0.0, 1.0) {}
    ...
    int operator()(std::mt19937& gen) const
    {
        ...
        // x = lambda * (-ln(U)) ^ (1 / k)
        double lifespan = _lambda * std::pow(-std::log(1.0 - U), 1.0 / _k);
